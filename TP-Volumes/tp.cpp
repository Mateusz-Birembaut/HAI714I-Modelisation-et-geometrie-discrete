...
void Mesh::simplify(unsigned int resolution) {
    if (resolution <= 0)
    {
        std::cerr << "rosulution <= 0" << std::endl;
        return;
    }

    // Compute the cube C that englobes all vertices
    Vec3 bottomLeftBox = Vec3(FLT_MAX, FLT_MAX, FLT_MAX);
    Vec3 topRightBox = Vec3(-FLT_MAX, -FLT_MAX, -FLT_MAX);

    //std::cerr << "Parcourt des sommet pour la bounding box." << std::endl;
    for (size_t i = 0; i < this->vertices.size(); i++){ // pour chaque sommets
        for (size_t j = 0; j < 3; j++){
            if (this->vertices[i][j] < bottomLeftBox[j]){ // je regarde si la coordonnée du sommet est plus petite que la coordonnée du coin inférieur gauche
                bottomLeftBox[j] = this->vertices[i][j] - 0.0001; // pour eviter impression de sommets sur les bords
            }
            if (this->vertices[i][j] > topRightBox[j]){ // l'inverse ici
                topRightBox[j] = this->vertices[i][j] + 0.0001;
            }
            
        }
        
    } // a la fin on a deux points pour la boite englobante
    
    // Create a grid of size resolution x resolution x resolution in the cube

    Grid grid;
    grid.resolution = resolution;
    grid.minPos = bottomLeftBox;
    grid.maxPos = topRightBox;
    grid.cells = std::vector<GridData>(resolution * resolution * resolution);

    // For each vertex, add the position and normal to its representant in the grid
    // (Count the number of vertices per cell)

    //std::cerr << " Parcourt des sommets pour la grille " << std::endl;
    for (size_t i = 0; i < this->vertices.size(); i++){
        int index = grid.getIndex(this->vertices[i]);
        grid.cells[index].position += this->vertices[i];
        grid.cells[index].normal += this->normals[i];
        grid.cells[index].count++;
    }
    ...
    //std::cerr << " Parcourt des triangles a réindexer ou supprimer " << std::endl;

    for (size_t i = 0; i < this->triangles.size(); i++){

        // Récupérer les 3 indices de cellules
        int index1 = grid.getIndex(vertices[triangles[i][0]]);
        int index2 = grid.getIndex(vertices[triangles[i][1]]);
        int index3 = grid.getIndex(vertices[triangles[i][2]]);
        
        // Si tous différents -> réindexer
    ...
    //std::cerr << " Suppression des triangles " << std::endl;
    while (!to_be_erased.empty()) {
        this->triangles.erase(this->triangles.begin() + to_be_erased.back());
        to_be_erased.pop_back();
    }

    // Divide the position of each representant by the number of vertices in the cell.
    // Normalize the normals

    //std::cerr << " Moyennage des normales et positions " << std::endl;
    for (size_t i = 0; i < grid.cells.size(); i++){
        grid.cells[i].position /= grid.cells[i].count;
        grid.cells[i].normal /= grid.cells[i].count;
        grid.cells[i].normal.normalize();
    }

    std::cout << " Avant : NB de sommets " << vertices.size() << std::endl;
    
    // creer maillage simplifié
    vertices.clear();
    normals.clear();

    // map pour la correspondance entre les indices de la grille et les indices des sommets
    std::map<int, int> gridToVertexIndex;
    int newIndex = 0;

    // parcours des cellules de la grille non vides
    for (size_t i = 0; i < grid.cells.size(); i++) {
        if (grid.cells[i].count > 0) {
            gridToVertexIndex[i] = newIndex++;
            vertices.push_back(grid.cells[i].position);
            normals.push_back(grid.cells[i].normal);
        }
    }

    // reindexer les triangles vers les indices des sommets correctes
    for (Triangle& triangle : triangles) {
        triangle[0] = gridToVertexIndex[triangle[0]];
        triangle[1] = gridToVertexIndex[triangle[1]];
        triangle[2] = gridToVertexIndex[triangle[2]];
    }

    std::cout << "Apres : NB de sommets " << vertices.size() << std::endl;
...
void Mesh::adaptativeSimplify(unsigned int numOfPerLeafVertices) {
    ...
    //std::cerr << "Defini la 1ere boite englobante" << std::endl;
    for (size_t i = 0; i < this->vertices.size(); i++){ // pour chaque sommets
        for (size_t j = 0; j < 3; j++){
            if (this->vertices[i][j] < bottomLeftBox[j]){ // je regarde si la coordonnée du sommet est plus petite que la coordonnée du coin inférieur gauche
                bottomLeftBox[j] = this->vertices[i][j] - 0.0001; // pour eviter impression de sommets sur les bords
    ...
    //std::cerr << "Création Octree" << std::endl;
    octree = Octree(numOfPerLeafVertices, bottomLeftBox, topRightBox);
    for (size_t i = 0; i < vertices.size(); i++){
        octree.addPoint(*octree.base_octant, vertices[i], normals[i]);
    }

    // Compute the mean position and normal for each leaf's representant
    octree.SummPosAndNormal();
    ...
    for (size_t i = 0; i < this->triangles.size(); i++){
        ...
        // Récupérer les 3 indices de cellules
        int index1 = octree.getOctantIndex(vertices[triangles[i][0]]);
        int index2 = octree.getOctantIndex(vertices[triangles[i][1]]);
        int index3 = octree.getOctantIndex(vertices[triangles[i][2]]);
    ...
    // map pour la correspondance entre les indices de la grille et les indices des sommets
    std::map<int, int> octreeToVertexIndex;
    int newIndex = 0;

    // parcours des feuilles de l'octree
    // pour tous mes triangles
    // get id unique de l'octant pour chaque sommet
    // si tous les sommets ont le même id -> réindexer
    // sinon supprimer le triangle
    octree.traverseLeaves(*octree.base_octant, octreeToVertexIndex, newIndex, vertices, normals);

    // reindexer les triangles vers les indices des sommets correctes
    for (Triangle& triangle : triangles) {
        triangle[0] = octreeToVertexIndex[triangle[0]];
        triangle[1] = octreeToVertexIndex[triangle[1]];
        triangle[2] = octreeToVertexIndex[triangle[2]];
    }
...
bool quadsCalculated = false;

void drawVoxels( std::vector<Quad>& quads ){
    for (const auto& quad : quads) {
        displayVoxel(quad.centre, quad.sideLength);
    }
}
...
void draw () {
    ...
    if(octree.base_octant != nullptr && display_octree && octree_displayed){
        glDisable(GL_LIGHTING);
        glColor3f(1.0f, 0.0f, 0.0f); // Rouge pour l'octree
        //octree.draw();
    }
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'd':
        ...
        std::cout << "nodesPerLeafInOctree: " << nodesPerLeafInOctree << std::endl;
        break;
    case 'e':
        nodesPerLeafInOctree = std::max(1, nodesPerLeafInOctree + 1);
        std::cout << "nodesPerLeafInOctree: " << nodesPerLeafInOctree << std::endl;
...
int main (int argc, char ** argv) {
    ...
    //displayVoxelSphere(Vec3(0.0, 0.0, 0.0), 1, 6, SURFACIQUE, quads);

    //displayVoxelCylinder(Vec3(0.0, -1.0, -1.0), Vec3(0, 2.0, 2.0), 0.6, 40, SURFACIQUE , quads);

    //display_INTERSECTION_SphereCylinder(Vec3(0.0, 0.0, 0.0), 0.6, Vec3(0.0, 0.0, 0.0), Vec3(0.0, 2.0, 0.0), 0.25, 50);
    //display_SOUSTRACTION_SphereCylinder(Vec3(0.0, 0.0, 0.0), 0.6, Vec3(0.0, 0.0, -1.0), Vec3(0.0, 0.0, 2.0), 0.25, 50);
    //display_UNION_SphereCylinder(Vec3(0.0, 0.0, 0.0), 0.6, Vec3(0.0, 0.0, -1.0), Vec3(0.0, 0.0, 2.0), 0.25, 50);

    display_SphereCylinderOperation(Vec3(0.0, 0.0, 0.0), 1., Vec3(0.0, -1.3, -2.0), Vec3(0.0, 2.0, 4.0), 0.7, 50, SOUSTRACTION, quads);
